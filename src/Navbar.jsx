import { Link } from 'react-router-dom';
import { CgHome } from "react-icons/cg";
import { useEffect, useState } from 'react';
import { getPathFoto } from './api/usuario';

function Navbar({image}) {

    const [imagePerfil, setImagePerfil] = useState(null);

    useEffect(()=>{
        console.log("Navbar id: ", image)
        
        getPathFoto(image)
        .then(item => {
            console.log("Item ", item.fileName)
            setImagePerfil(item.fileName);
        })
        .catch((err) => {
            console.log(err.message);
        });
    },[])

    return (
        <nav className="navbar">
            <div>
                <svg viewBox="0 0 24 24" fill="#005ce6" width="30px" height="30px" class="x1lliihq x2lah0s x1k90msu x2h7rmj x1qfuztq x1fey0fg xy75621 xni59qk"><path d="M22 12.037C22 6.494 17.523 2 12 2S2 6.494 2 12.037c0 4.707 3.229 8.656 7.584 9.741v-6.674H7.522v-3.067h2.062v-1.322c0-3.416 1.54-5 4.882-5 .634 0 1.727.125 2.174.25v2.78a12.807 12.807 0 0 0-1.155-.037c-1.64 0-2.273.623-2.273 2.244v1.085h3.266l-.56 3.067h-2.706V22C18.164 21.4 22 17.168 22 12.037z"></path></svg>
            </div>
            <div>
                <div><Link to="/Inicio"><CgHome size="25px" className='icono-casa'/></Link></div>
            </div>
            <div>
                <div style={{color: 'white'}}><Link to="/Perfil">{imagePerfil}</Link></div>
            </div>
            
        </nav>
    );
};

export default Navbar;